...
const A3Report: React.FC<Props> = ({ project, onBack }) => {
  ...
  return (
    <div className="max-w-7xl mx-auto pb-24 px-4 sm:px-6">
      ...
      <div className="a3-page-wrapper bg-white dark:bg-slate-900 p-8 sm:p-10 shadow-2xl border border-slate-200 dark:border-slate-800 rounded-3xl min-h-[950px]">
          
          {/* Header section with Metadata and branding */}
          <div className="grid grid-cols-1 md:grid-cols-4 border-4 border-slate-900 dark:border-slate-850 mb-6 rounded-lg overflow-hidden shrink-0">
              ...
              <div className="p-5 border-t md:border-t-0 md:border-r border-slate-900 dark:border-slate-855 bg-slate-50 dark:bg-slate-950 flex flex-col justify-between">
                  <div className="text-[10px] text-slate-500 dark:text-slate-400 font-black tracking-wider uppercase">Fasgodkännande / Tollgates</div>
                  <div className="grid grid-cols-5 gap-1 mt-2">
                    {Object.values(Phase).map((ph, idx) => {
                      ...
                      const colorClass = status === 'Approved' 
                        ...
                        : 'bg-slate-200 text-slate-450 dark:bg-slate-800 dark:text-slate-500';
                      return (
                        <div key={idx} className="flex flex-col items-center">
                          <div className={`w-6 h-6 rounded-full ${colorClass} flex items-center justify-center text-[10px] font-black shadow-sm`}>
                            {ph[0]}
                          </div>
                          <span className="text-[8px] mt-1 text-slate-500 dark:text-slate-400 font-bold">{ph}</span>
                        </div>
                      );
                    })}
                  </div>
              </div>
              <div className="p-5 border-t md:border-t-0 bg-slate-50 dark:bg-slate-955 flex flex-col justify-between gap-4">
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      <div className="text-[8px] text-slate-400 uppercase font-black tracking-wider">Projektledare</div>
                      <div className="text-xs font-extrabold text-slate-800 dark:text-slate-200">Ibrahim Qallaki</div>
                    </div>
                    <div>
                      <div className="text-[8px] text-slate-400 uppercase font-black tracking-wider">Metodik</div>
                      <div className="text-xs font-extrabold text-blue-700 dark:text-blue-400">Six Sigma Belt</div>
                    </div>
                  </div>
                  <div>
                    <div className="text-[8px] text-slate-400 uppercase font-black tracking-wider">Utskriftsdatum</div>
                    <div className="text-xs font-bold text-slate-705 dark:text-slate-300 font-mono">{new Date().toLocaleDateString('sv-SE', { year: 'numeric', month: 'long', day: 'numeric' })}</div>
                  </div>
              </div>
          </div>
          ...
          <div className="print-grid-layout grid grid-cols-1 lg:grid-cols-2 gap-6">
              ...
              <div className="space-y-6">
                  ...
                  <section className="print-card-border border-2 border-slate-900 dark:border-slate-850 p-5 rounded-xl bg-slate-50/20 dark:bg-slate-950/20 shadow-sm relative overflow-hidden text-slate-900 dark:text-white">
                      <div className="absolute right-4 top-4 text-slate-100 print-hidden">
                        <Target className="w-10 h-10 text-slate-300 dark:text-slate-800" />
                      </div>
                      ...
                      <div className="space-y-4">
                          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                              <div>
                                  ...
                                  <p className="text-xs text-slate-705 dark:text-slate-300 leading-relaxed font-medium bg-white dark:bg-slate-950 p-2.5 rounded border border-slate-200 dark:border-slate-800 min-h-[50px]">
                          ...
                          <div className="grid grid-cols-3 gap-3">
                              <div className="bg-white dark:bg-slate-900 p-2 border border-slate-200 dark:border-slate-800 rounded">
                                  ...
                                  <div className="text-xs font-bold text-slate-800 dark:text-slate-200 truncate mt-0.5">{project.goal || 'Ej specificerat'}</div>
                              </div>
                              <div className="bg-white dark:bg-slate-900 p-2 border border-slate-200 dark:border-slate-800 rounded">
                                  <div className="text-[8px] text-slate-400 uppercase font-black tracking-wider">Omfattning (Scope)</div>
                                  <div className="text-xs font-bold text-slate-800 dark:text-slate-200 truncate mt-0.5">{project.scope || 'Inkl / Exkl saknas'}</div>
                              </div>
                              <div className="bg-white dark:bg-slate-900 p-2 border border-slate-200 dark:border-slate-800 rounded">
                                  <div className="text-[8px] text-slate-400 uppercase font-black tracking-wider">Sponsor/Intressenter</div>
                                  <div className="text-xs font-bold text-slate-800 dark:text-slate-200 truncate mt-0.5">{project.stakeholders || 'Sponsor saknas'}</div>
                              </div>
                          </div>

                          {/* SIPOC Overview Subtable */}
                          <div className="mt-3 pt-3 border-t border-slate-200/60 dark:border-slate-800/80">
                              <h4 className="text-[10px] font-black text-slate-600 dark:text-slate-400 uppercase tracking-wider mb-2">Övergripande Flödesmatris (SIPOC)</h4>
                              <div className="grid grid-cols-5 gap-1.5 text-center text-[9px]">
                                  <div className="bg-indigo-50 dark:bg-indigo-950/20 border border-indigo-150 dark:border-indigo-900/40 p-1.5 rounded">
                                      <div className="font-bold text-indigo-800 dark:text-indigo-400">S (Leverantör)</div>
                                      <div className="text-slate-600 dark:text-slate-400 mt-1 line-clamp-2">{sipoc.suppliers.slice(0,2).join(', ') || '-'}</div>
                                  </div>
                                  <div className="bg-blue-50 dark:bg-blue-950/20 border border-blue-150 dark:border-blue-900/40 p-1.5 rounded">
                                      <div className="font-bold text-blue-800 dark:text-blue-400">I (Insignal)</div>
                                      <div className="text-slate-600 dark:text-slate-400 mt-1 line-clamp-2">{sipoc.inputs.slice(0,2).join(', ') || '-'}</div>
                                  </div>
                                  <div className="bg-amber-50 dark:bg-amber-950/20 border border-amber-150 dark:border-amber-900/40 p-1.5 rounded font-black">
                                      <div className="font-bold text-amber-800 dark:text-amber-400">P (Process)</div>
                                      <div className="text-slate-600 dark:text-slate-400 mt-1 line-clamp-2 font-normal">{sipoc.process.slice(0,2).join(', ') || '-'}</div>
                                  </div>
                                  <div className="bg-green-50 dark:bg-green-950/20 border border-green-150 dark:border-green-900/40 p-1.5 rounded">
                                      <div className="font-bold text-green-800 dark:text-green-400">O (Utdata)</div>
                                      <div className="text-slate-600 dark:text-slate-400 mt-1 line-clamp-2">{sipoc.outputs.slice(0,2).join(', ') || '-'}</div>
                                  </div>
                                  <div className="bg-slate-50 dark:bg-slate-900/40 border border-slate-200 dark:border-slate-800 p-1.5 rounded">
                                      <div className="font-bold text-slate-800 dark:text-slate-400">C (Kund)</div>
                                      <div className="text-slate-600 dark:text-slate-400 mt-1 line-clamp-2">{sipoc.customers.slice(0,2).join(', ') || '-'}</div>
                                  </div>
                              </div>
                          </div>
                      </div>
                  ...
                  <section className="print-card-border border-2 border-slate-900 dark:border-slate-850 p-5 rounded-xl bg-slate-50/20 dark:bg-slate-950/20 shadow-sm relative text-slate-900 dark:text-white">
                      <div className="absolute right-4 top-4 text-slate-100 print-hidden">
                      ...
                      <div className="grid grid-cols-3 gap-3 mb-4">
                          <div className="bg-white dark:bg-slate-950 p-3 rounded border border-slate-200 dark:border-slate-800 text-center shadow-xs">
                              <span className="text-[9px] text-slate-400 uppercase font-black text-slate-500 dark:text-slate-400">Antal mätningar (N)</span>
                              <div className="text-xl font-black text-slate-900 dark:text-white mt-1">{n}</div>
                          </div>
                          <div className="bg-white dark:bg-slate-950 p-3 rounded border border-slate-200 dark:border-slate-800 text-center shadow-xs">
                              <span className="text-[9px] text-slate-400 uppercase font-black text-slate-500 dark:text-slate-400">Aritmetiskt Medel (μ)</span>
                              <div className="text-xl font-black text-slate-900 dark:text-white mt-1">{n > 0 ? mean.toFixed(2) : '0.00'}</div>
                          </div>
                          <div className="bg-white dark:bg-slate-950 p-3 rounded border border-slate-200 dark:border-slate-800 text-center shadow-xs">
                              <span className="text-[9px] text-slate-400 uppercase font-black text-slate-500 dark:text-slate-400">Std Avvikelse (σ)</span>
                              <div className="text-xl font-black text-slate-900 dark:text-white mt-1">{n > 1 ? stdDev.toFixed(3) : '0.000'}</div>
                          </div>
                      </div>
                      ...
                      <div className="grid grid-cols-2 gap-4">
                          ...
                          <div className="bg-white dark:bg-slate-950 p-3 border border-slate-200 dark:border-slate-800 rounded-lg flex flex-col justify-between">
                              <div>
                                <span className="text-[9px] text-slate-400 dark:text-slate-500 uppercase font-black tracking-wider">Processkartläggning (Värdeflöde)</span>
                                  ...
                                  {processSteps.length > 0 ? (
                                    processSteps.slice(0, 3).map((step: any, i: number) => (
                                      <div key={i} className="flex items-center gap-1.5">
                                        ...
                                        <span className="font-medium text-slate-700 dark:text-slate-300 truncate max-w-[150px]">{step.name}</span>
                                      </div>
                                    ))
                                  ) : (
                                    <div className="text-[9px] text-slate-400 dark:text-slate-500 italic">Inga process-steg tillgängliga.</div>
                                  )}
                                  {processSteps.length > 3 && (
                                    <div className="text-[8px] text-slate-400 dark:text-slate-500 mt-1">+ {processSteps.length - 3} ytterligare steg i kartan</div>
                                  )}
                                </div>
                              </div>
                              <span className="text-[8px] text-slate-500 dark:text-slate-400 mt-2 font-mono">Total processlängd registrerad.</span>
                          </div>
                      </div>
                  </section>

                  {/* PHASE 3: MEASURE COMPLEMENTS - DATA COLLECTION NOTES */}
                  <section className="print-card-border border border-slate-200 dark:border-slate-800 p-4 rounded-xl bg-white dark:bg-slate-950/10 shadow-xs text-slate-900 dark:text-white">
                      <div className="flex gap-4 items-start">
                          <div className="p-2.5 bg-blue-50 dark:bg-blue-900/40 text-blue-600 dark:text-blue-400 rounded-lg shrink-0">
                              <Layers className="w-5 h-5" />
                          </div>
                          <div>
                              <h4 className="text-[10px] font-black text-slate-500 dark:text-slate-400 uppercase tracking-widest mb-1">Mätsystemsanalys (MSA) & Mätnoggrannhet</h4>
              ...
              <div className="space-y-6">
                  ...
                  <section className="print-card-border border-2 border-slate-900 dark:border-slate-850 p-5 rounded-xl bg-slate-50/20 dark:bg-slate-950/20 shadow-sm relative text-slate-900 dark:text-white">
                      <div className="absolute right-4 top-4 text-slate-100 print-hidden">
                        <Wrench className="w-10 h-10 text-slate-300 dark:text-slate-800" />
                      </div>
                      <h3 className="text-xs font-black bg-slate-900 text-white px-3 py-1.5 inline-block mb-4 uppercase tracking-widest rounded print-badge-bg">
                      ...
                      <div className="space-y-4">
                          {/* 5 Whys Chain */}
                          <div className="bg-white dark:bg-slate-950 p-3 border border-slate-200 dark:border-slate-800 rounded-lg">
                              <div className="text-[9px] text-slate-400 dark:text-slate-550 uppercase font-black tracking-wider mb-2">5 Varför Causal Chain (Rotorsakskedja)</div>
                              {whyChain.length > 0 ? (
                                  <div className="space-y-1.5">
                                      {whyChain.map((level, i) => (
                                          <div key={i} className="flex items-center gap-1.5">
                                              <div className="text-[9px] font-black text-rose-600 bg-rose-50 dark:bg-rose-950/40 px-1.5 py-0.5 rounded border border-rose-100 dark:border-rose-900/30 shrink-0 w-28 text-center truncate">
                                                  {level.label}
                                              </div>
                                              <ArrowRight className="w-2.5 h-2.5 text-slate-300 dark:text-slate-700 shrink-0" />
                                              <div className="text-[10px] text-slate-700 dark:text-slate-300 italic truncate font-medium">
                                                  "{level.text}"
                                              </div>
                                          </div>
                                      ))}
                                  ...
                                  <div className="text-[10px] text-slate-400 dark:text-slate-500 italic font-medium p-1">
                          ...
                          <div>
                              <div className="text-[9px] text-slate-400 dark:text-slate-500 uppercase font-black tracking-wider mb-2">Fiskbens-analys (Ishikawa / 6M)</div>
                              <div className="grid grid-cols-2 gap-3 text-[10px]">
                                {ishikawa.length > 0 ? (
                                  ishikawa.map((cat: any, i: number) => (
                                    cat.causes.length > 0 && (
                                      <div key={i} className="bg-white dark:bg-slate-950 p-2 border border-slate-150 dark:border-slate-800 rounded text-slate-800 dark:text-slate-200">
                                        <div className="font-extrabold text-slate-800 dark:text-slate-200 uppercase text-[9px] border-b border-slate-100 dark:border-slate-800 pb-0.5 mb-1 flex justify-between items-center bg-slate-50/50 dark:bg-slate-900/30 px-1 rounded">
                                          <span>{cat.name}</span>
                                          <span className="text-[8px] text-slate-400 dark:text-slate-500">({cat.causes.length})</span>
                                        </div>
                                  ...
                                  <div className="col-span-2 text-slate-400 dark:text-slate-500 text-center py-4 bg-white dark:bg-slate-950 rounded border border-dashed border-slate-200 dark:border-slate-800">
                  ...
                  <section className="print-card-border border-2 border-slate-900 dark:border-slate-850 p-5 rounded-xl bg-slate-50/20 dark:bg-slate-950/20 shadow-sm relative text-slate-800 dark:text-white">
                      <div className="absolute right-4 top-4 text-slate-100 print-hidden">
                        <AlertTriangle className="w-10 h-10 text-slate-300 dark:text-slate-800" />
                      </div>
                      ...
                      <div className="space-y-4">
                          ...
                          <div>
                              <div className="text-[9px] text-slate-400 dark:text-slate-500 uppercase font-black tracking-wider mb-2">FMEA Riskprioritering (Största processrisker)</div>
                              {sortedFMEA.length > 0 ? (
                                    ...
                                    <table className="w-full text-left text-[9px]">
                                        ...
                                        <tbody className="bg-white dark:bg-slate-950">
                                            {sortedFMEA.map((row: any, i: number) => (
                                                <tr key={i} className="border-b border-slate-100 dark:border-slate-900 font-medium">
                                                    <td className="p-1 px-2 text-slate-850 dark:text-slate-200 truncate max-w-[200px]" title={`${row.step}: ${row.failureMode}`}>
                                                      <span className="font-extrabold text-slate-500 mr-1">{row.step || 'Steg'}</span>
                                                      {row.failureMode}
                                                    </td>
                                                    <td className="p-1 text-center text-slate-600 dark:text-slate-400">{row.severity}</td>
                                                    <td className="p-1 text-center text-slate-600 dark:text-slate-400">{row.occurrence}</td>
                                                    <td className="p-1 text-center text-slate-600 dark:text-slate-400">{row.detection}</td>
                                                    <td className="p-1 text-center font-extrabold text-rose-700 dark:text-rose-400 bg-rose-50/40 dark:bg-rose-950/20">{row.rpn}</td>
                                                </tr>
                                            ))}
                                        </tbody>
                                    </table>
                                ...
                                <div className="text-[10px] text-slate-400 dark:text-slate-500 italic bg-white dark:bg-slate-950 p-2 text-center rounded border border-dashed border-slate-200 dark:border-slate-800">
                          ...
                          {/* Improvements List */}
                          <div>
                              <div className="text-[9px] text-slate-400 dark:text-slate-500 uppercase font-black tracking-wider mb-2">Genomförda & Planerade Åtgärder</div>
                              <div className="grid grid-cols-1 md:grid-cols-2 gap-2 text-[10px]">
                                {project.improvements && project.improvements.length > 0 ? (
                                  project.improvements.slice(0, 4).map((imp) => (
                                    <div key={imp.id} className="bg-white dark:bg-slate-950 p-2.5 rounded border border-slate-200 dark:border-slate-800 flex items-center justify-between gap-1 text-slate-805 dark:text-slate-200">
                                      <div className="flex items-center gap-2 truncate">
                                        <CheckCircle2 className={`w-3.5 h-3.5 shrink-0 ${imp.status === 'Done' ? 'text-emerald-500' : 'text-slate-300 dark:text-slate-700'}`} />
                                        <span className="text-slate-800 dark:text-slate-200 font-bold truncate">{imp.action}</span>
                                      </div>
                                      <span className={`text-[8px] font-black px-1.5 py-0.5 rounded select-none shrink-0 uppercase ${
                                        imp.status === 'Done' ? 'bg-emerald-100 dark:bg-emerald-950 text-emerald-800 dark:text-emerald-400' : 'bg-amber-100 dark:bg-amber-950 text-amber-800 dark:text-amber-400'
                                      }`}>
                                  ...
                                  <div className="col-span-2 text-slate-400 dark:text-slate-500 text-center py-3 bg-white dark:bg-slate-950 rounded border border-dashed border-slate-200 dark:border-slate-800">
                  ...
                  <section className="print-card-border border border-slate-900 dark:border-slate-850 p-5 rounded-xl bg-slate-50/20 dark:bg-slate-950/20 shadow-sm relative text-slate-900 dark:text-white">
                      <div className="absolute right-4 top-4 text-slate-100 print-hidden">
                        <ShieldCheck className="w-10 h-10 text-slate-300 dark:text-slate-800" />
                      </div>
                      <h3 className="text-xs font-black bg-slate-900 text-white px-3 py-1.5 inline-block mb-4 uppercase tracking-widest rounded print-badge-bg">
                      ...
                      <div className="grid grid-cols-2 gap-4">
                          <div className="bg-white dark:bg-slate-955 p-3 rounded-lg border border-slate-250 dark:border-slate-800">
                              <h4 className="text-[10px] font-black text-slate-550 dark:text-slate-400 uppercase tracking-wider mb-2">Standardiserat Arbetssätt (SOP)</h4>
                              <p className="text-[10px] text-slate-650 dark:text-slate-400 leading-relaxed italic line-clamp-3">
                          ...
                          <div className="bg-white dark:bg-slate-955 p-3 rounded-lg border border-slate-250 dark:border-slate-800">
                              <h4 className="text-[10px] font-black text-slate-550 dark:text-slate-400 uppercase tracking-wider mb-2">Aktiv Kontrollplan (Control Plan)</h4>
                              <p className="text-[10px] text-slate-650 dark:text-slate-400 leading-relaxed italic line-clamp-3">
