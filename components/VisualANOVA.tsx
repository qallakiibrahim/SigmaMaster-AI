      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <div className="space-y-4">
          ...
          <div className="space-y-3">
            {groups.map((group) => (
              <div key={group.id} className="bg-white p-4 rounded-xl border border-slate-200 shadow-sm group relative">
                <div className="flex items-center gap-3 mb-3">
                  <input
                    type="text"
                    value={group.name}
                    onChange={(e) => updateGroupName(group.id, e.target.value)}
                    className="font-bold text-slate-800 bg-transparent border-b border-transparent focus:border-blue-500 outline-none"
                  />
                  <button
                ...
                <textarea
                  ...
                  className="w-full p-3 text-xs border border-slate-100 rounded-lg bg-slate-50 focus:ring-2 focus:ring-blue-500 outline-none min-h-[80px]"
            ...
            <button
              onClick={addGroup}
              className="w-full py-3 border-2 border-dashed border-slate-200 rounded-xl text-slate-400 hover:border-blue-300 hover:text-blue-500 transition-all flex items-center justify-center gap-2 text-xs font-bold uppercase"
            >
        ...
        <div className="space-y-4">
          ...
          {stats ? (
            <div className="bg-slate-900 text-white p-6 rounded-2xl shadow-xl space-y-6">
              <div className="grid grid-cols-2 gap-4">
                <div className="bg-white/10 p-4 rounded-xl">
                  <div className="text-[10px] text-slate-400 uppercase font-bold mb-1">F-Statistik</div>
                ...
                <div className={`p-4 rounded-xl ${stats.pValue < 0.05 ? 'bg-emerald-500/20' : 'bg-red-500/20'}`}>
                  <div className="text-[10px] text-slate-400 uppercase font-bold mb-1">P-Värde (ca)</div>
              ...
              <div className="space-y-3">
                <div className="flex justify-between text-xs border-b border-white/10 pb-2">
                  <span className="text-slate-400">Frihetsgrader (Between/Within)</span>
                  <span className="font-mono">{stats.dfBetween} / {stats.dfWithin}</span>
                </div>
                <div className="flex justify-between text-xs border-b border-white/10 pb-2">
                  <span className="text-slate-400">Mean Square Between (MSB)</span>
                  <span className="font-mono">{stats.msBetween.toFixed(3)}</span>
                </div>
                <div className="flex justify-between text-xs border-b border-white/10 pb-2">
                  <span className="text-slate-400">Mean Square Within (MSW)</span>
            ...
            <div className="bg-slate-50 border-2 border-dashed border-slate-200 rounded-2xl p-12 text-center">
              <p className="text-slate-400 text-sm">Mata in minst två grupper med data för att se ANOVA-analysen.</p>
            </div>
          )}
        </div>
      </div>
