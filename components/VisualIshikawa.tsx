    ...
    <div className="space-y-8">
      <div className="flex justify-between items-center">
        ...
        <button
          ...
          className="flex items-center gap-2 bg-purple-100 dark:bg-purple-950/40 text-purple-700 dark:text-purple-300 px-4 py-2 rounded-full hover:bg-purple-200 dark:hover:bg-purple-900/40 transition-all text-xs font-bold disabled:opacity-50"
        >
      ...
      {/* Visual Fishbone */}
      <div className="relative min-h-[500px] bg-slate-50 dark:bg-slate-900/40 rounded-3xl border border-slate-200 dark:border-slate-800 p-8 overflow-x-auto">
        <div className="min-w-[800px] h-full flex flex-col justify-center relative">
          
          {/* Central Spine */}
          <div className="absolute top-1/2 left-0 right-40 h-1 bg-slate-400 dark:bg-slate-700 -translate-y-1/2 rounded-full"></div>
          
          {/* Head (Problem) */}
          <div className="absolute top-1/2 right-0 -translate-y-1/2 w-40 h-24 bg-slate-800 dark:bg-slate-950 border dark:border-slate-800 text-white rounded-r-full flex items-center justify-center p-4 text-center shadow-xl z-10">
          ...
          <div className="grid grid-cols-3 gap-8 mb-32 relative z-0">
            {[0, 1, 2].map(idx => (
              <div key={idx} className="flex flex-col items-center">
                <div className="bg-white dark:bg-slate-900 px-4 py-2 rounded-lg border border-slate-200 dark:border-slate-800 shadow-sm font-bold text-xs text-slate-700 dark:text-slate-300 mb-4 w-full text-center">
                  {data.categories[idx].name}
                </div>
                <div className="w-px h-20 bg-slate-300 dark:bg-slate-700 relative">
                        ...
                        {data.categories[idx].causes.map((cause, cIdx) => (
                            <div key={cIdx} className="group flex items-center gap-1">
                                <input
                                    ...
                                    className="flex-1 bg-white dark:bg-slate-800 border border-slate-100 dark:border-slate-700 text-slate-700 dark:text-slate-200 rounded px-2 py-1 text-[10px] outline-none focus:border-blue-400 shadow-sm"
                                    placeholder="..."
                                />
                                <button onClick={() => removeCause(idx, cIdx)} className="opacity-0 group-hover:opacity-100 text-red-405 hover:text-red-500 transition-all">
                                    <Trash2 className="w-3 h-3" />
                                </button>
                            </div>
                        ))}
                        <button onClick={() => addCause(idx)} className="w-full py-1 border border-dashed border-slate-300 dark:border-slate-700 rounded text-[9px] text-slate-400 dark:text-slate-500 hover:text-blue-500 dark:hover:text-blue-400 hover:border-blue-300 dark:hover:border-blue-700 transition-all">
                            + Orsak
                        </button>
                    </div>
                </div>
          ...
          <div className="grid grid-cols-3 gap-8 mt-4 relative z-0">
            {[3, 4, 5].map(idx => (
              <div key={idx} className="flex flex-col-reverse items-center">
                <div className="bg-white dark:bg-slate-900 px-4 py-2 rounded-lg border border-slate-200 dark:border-slate-800 shadow-sm font-bold text-xs text-slate-700 dark:text-slate-300 mt-4 w-full text-center">
                  {data.categories[idx].name}
                </div>
                <div className="w-px h-20 bg-slate-300 dark:bg-slate-700 relative">
